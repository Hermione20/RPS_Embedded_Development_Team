...
void USART_StructInit(USART_InitTypeDef* USART_InitStruct);//给结构体变量赋默认值
void USART_ClockInit(USART_TypeDef* USARTx, USART_ClockInitTypeDef* USART_ClockInitStruct);//用来配置同步时钟的
void USART_ClockStructInit(USART_ClockInitTypeDef* USART_ClockInitStruct);//用来配置同步时钟的
...
void USART_SendData(USART_TypeDef* USARTx, uint16_t Data);//发送数据（写DR）
uint16_t USART_ReceiveData(USART_TypeDef* USARTx);//接收数据（读DR）
...
void USART_ITConfig(USART_TypeDef* USARTx, uint16_t USART_IT, FunctionalState NewState);//使能中断输出信号
