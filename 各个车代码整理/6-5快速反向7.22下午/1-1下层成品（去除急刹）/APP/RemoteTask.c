...
//u8 auto_sup_cha_key_flag = 0;
//u32 auto_sup_cha_change_time = 0;
...
void Mouse_Key_Rotate_Reverse_Control(void)
{
  switch (chassis.ctrl_mode)
    ...
    case MANUAL_FOLLOW_GIMBAL:
    {
      if((RC_CtrlData.key.v&KEY_CTRL)&&(rotate_key_flag == 0))
        {
    ...
    case CHASSIS_ROTATE:
    {
      if((RC_CtrlData.key.v&KEY_CTRL)&&(rotate_key_flag == 0)&&(close_rotate_flag==0))
        {
          // 底盘云台相对角度给到下一个360的倍数
          rotate_num = GMYawEncoder.ecd_angle/180 ;
          close_rotate_flag = 1;
          rotate_key_flag = 1;
        }

      if((close_rotate_flag==1)&&(GMYawEncoder.ecd_angle-((rotate_num+1)*180)<=35)&&(GMYawEncoder.ecd_angle-((rotate_num+1)*180)>=-35))
        {
          chassis.ctrl_mode = MANUAL_FOLLOW_GIMBAL;
          chassis_rotate_flag ^=1;
          chassis.position_ref = (rotate_num+1)*180;
          close_rotate_flag = 0;
          rotate_num_ture=abs(rotate_num+1);
        }
      else if((close_rotate_flag==1)&&(GMYawEncoder.ecd_angle-((rotate_num-1)*180)<=35)&&(GMYawEncoder.ecd_angle-((rotate_num-1)*180)>=-35))
        {
          chassis.ctrl_mode = MANUAL_FOLLOW_GIMBAL;
          chassis_rotate_flag ^=1;
          chassis.position_ref = (rotate_num-1)*180;
      ...
      else if((close_rotate_flag==1)&&(GMYawEncoder.ecd_angle-(rotate_num*180)<=35)&&(GMYawEncoder.ecd_angle-(rotate_num*180)>=-35))
        {
          chassis.ctrl_mode = MANUAL_FOLLOW_GIMBAL;
          chassis_rotate_flag ^=1;
          chassis.position_ref = rotate_num*180;
          close_rotate_flag = 0;
...
//		if((RC_CtrlData.key.v&KEY_G)&&(reverse_key_flag == 0))
//		{
//
//			chassis.ctrl_mode = CHASSIS_CHANGE_REVERSE;
//			GimbalRef.yaw_angle_dynamic_ref +=180;
...
void MouseShootControl(Mouse *mouse,Key *key)
{
  ...
  switch(friction_wheel_state)
    ...
    case FRICTION_WHEEL_ON: //启动完成
    {
      ...
      if(shot.ctrl_mode==1)
        {
          shot.ref_shot_bullets = shot.shot_bullets;
  ...
  /*********普通模式和自瞄模式切换状态机*************/
  if(autoshoot_mode == NORMAL_SHOOT)//普通模式
    {
      if(mouse->press_r)//鼠标右键进入自瞄模式
        {
  ...
  if(gim.ctrl_mode!= GIMBAL_INIT&&(gim.auto_ctrl_cmd!=CMD_TARGET_NUM))
    {
      ...
      if((key->v&KEY_F)&&(key_F_flag==0)&&(key_F_push_flag == 0))
        {
          key_F_push_flag = 1;
          key_f_pitch++;
          key_F_flag = 1;
        }
      else if((key->v&KEY_F)&&(key_F_flag==1)&&(key_F_push_flag == 0))            //
        {
          key_F_push_flag = 1;
          key_f_pitch++;
          key_F_flag = 0;
      ...
      if((key->v&KEY_E)&&(key_E_flag==0)&&(key_E_push_flag == 0))
        {
          key_E_push_flag = 1;
          key_e_yaw++;
          key_E_flag = 1;
        }
      else if((key->v&KEY_E)&&(key_E_flag==1)&&(key_E_push_flag == 0))            //
        {
          key_E_push_flag = 1;
          key_e_yaw++;
          key_E_flag = 0;
      ...
      if((key->v&KEY_V)&&(key_V_flag==0)&&(key_V_push_flag == 0))            //xiaofu
        {
          small_buff=1;
          key_V_push_flag = 1;
					rotate_num=0;
      ...
      GimbalRef.yaw_angle_dynamic_ref   += mouse->x* MOUSE_TO_YAW_ANGLE_INC_FACT;		 //鼠标X轴向右为正，因此‘+’保证向右为正
...
//	frictionRamp_Off.SetScale(&frictionRamp_Off, FRICTION_RAMP_OFF_TICK_COUNT);
  chassisRamp.SetScale(&chassisRamp, CHASSIS_RAMP_TICK_COUNT);
...
{
//	
// vx = ChassisSpeedRef.left_right_ref;
// vy = ChassisSpeedRef.forward_back_ref;
