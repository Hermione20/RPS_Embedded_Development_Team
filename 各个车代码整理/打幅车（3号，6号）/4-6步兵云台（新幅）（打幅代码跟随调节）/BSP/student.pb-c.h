...
typedef enum _Student__Catch {
  /*
   *必须从0开始
   */
  STUDENT__CATCH__userError = 0,
  STUDENT__CATCH__passwordError = 1
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(STUDENT__CATCH)
} Student__Catch;
...
struct  _Student
{
  ...
  uint32_t num;
  uint32_t c_score;
  Student__Catch error;
};
#define STUDENT__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&student__descriptor) \
    , (char *)protobuf_c_empty_string, 0, 0, STUDENT__CATCH__userError }
...
extern const ProtobufCEnumDescriptor    student__catch__descriptor;
