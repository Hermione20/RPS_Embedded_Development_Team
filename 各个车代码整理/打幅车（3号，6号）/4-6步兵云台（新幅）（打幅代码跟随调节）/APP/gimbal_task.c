...
//标识3号码步兵
    ...
    PID_struct_init(&pid_pit, POSITION_PID, 500,4,
                     15,0.01f,8); //25,0.1f,20); //8.0f, ,40    15,0.003f,5   18    16  5    //25 0.01 20
    PID_struct_init(&pid_pit_speed, POSITION_PID, 27000, 20000,
                    170, 0.001f, 60); //200, 0.3f, 18);//   --160 8 0    ->  180 10 50  ->190 12 70  ->120 8 0-》180.0f, 10.0f, 30 -> 235, 0.002f, 45 // 280,1,30
										//15 0.01 0...120 0 0       180 0.2 20    
    //------------------------------------------------
     PID_struct_init(&pid_yaw, POSITION_PID, 500, 4,
		                3,0.15f,8);//15, 0.1f, 25); //10.0f, 0, 100  13
     PID_struct_init(&pid_yaw_speed, POSITION_PID, 29000, 10000,
                    150,0.8f,40);//250,0,10);//250.0f,6.0f, 50-->300.0f,5.0f, 70
...
//----------------小陀螺P轴陀螺仪------------------------
    PID_struct_init(&pid_pit, POSITION_PID, 500,100,
                    15,0.003f,5); //15,0.01f,8); //8.0f, ,40       18    16  5    //25 0.01 20 // 25,0.1f,5j
    ...
    //------------------------------------------------
     PID_struct_init(&pid_yaw, POSITION_PID, 500, 10,
		                10.0f,0,100);//15, 0.1f, 25); // 28,0.1f,40 13
     PID_struct_init(&pid_yaw_speed, POSITION_PID, 29000, 10000,
...
////标识5号码步兵
  ...
  PID_struct_init(&pid_yaw, POSITION_PID, 1000, 4,
                  15, 0.17, 50); //7 0 10
...
//    8.4调
  PID_struct_init(&pid_pit_small_buff, POSITION_PID, 70, 20,
                  25.0f, 0.3f, 0);  //60  60 16 0.25 150           28 0.5 160
  PID_struct_init(&pid_pit_speed_small_buff, POSITION_PID, 25000, 20000,
                  350.0f, 7.0f, 0);//   
  PID_struct_init(&pid_yaw_small_buff, POSITION_PID, 60, 20,
                  15.0f, 0.3f, 20); //               18 0.4 180
