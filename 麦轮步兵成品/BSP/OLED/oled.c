...
void Write_IIC_Byte_OLED(unsigned char IIC_Byte)
{
	unsigned char i;
	unsigned char m,da;
	da=IIC_Byte;
...
// IIC Write Command
**********************************************/
void Write_IIC_Command_OLED(unsigned char IIC_Command)
{
	IIC_Start_OLED();
	Write_IIC_Byte_OLED(0x78);            //Slave address,SA0=0
	IIC_Wait_Ack_OLED();	
	Write_IIC_Byte_OLED(0x00);			//write command
	IIC_Wait_Ack_OLED();	
	Write_IIC_Byte_OLED(IIC_Command); 
	IIC_Wait_Ack_OLED();	
	IIC_Stop_OLED();
}
/**********************************************
// IIC Write Data
**********************************************/
void Write_IIC_Data_OLED(unsigned char IIC_Data)
{
	IIC_Start_OLED();
	Write_IIC_Byte_OLED(0x78);			//D/C#=0; R/W#=0
	IIC_Wait_Ack_OLED();	
	Write_IIC_Byte_OLED(0x40);			//write data
	IIC_Wait_Ack_OLED();	
	Write_IIC_Byte_OLED(IIC_Data);
	IIC_Wait_Ack_OLED();	
	IIC_Stop_OLED();
}
void OLED_WR_Byte(unsigned dat,unsigned cmd)
{
   ...
   Write_IIC_Data_OLED(dat);
   
		}
	else 
	{
   Write_IIC_Command_OLED(dat);
...
{ 	
 	...
 	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);	 //Ê¹ÄÜPC,D,G¶Ë¿ÚÊ±ÖÓ

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14|GPIO_Pin_15;	 //PD3,PD6ÍÆÍìÊä³ö  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 		 //Êä³ö
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//ÍÆÍìÊä³ö
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//ËÙ¶È50MHz
 	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //³õÊ¼»¯GPIOD3,6
 	GPIO_SetBits(GPIOB,GPIO_Pin_14|GPIO_Pin_15);	//PD3,PD6 Êä³ö¸ß
    delay_ms(200);
