...
volatile float mygetqval[9];	//用于存放传感器转换结果的数组  最后三个是磁力计的
static volatile float gx, gy, gz, ax, ay, az, mx, my, mz;   //作用域仅在此文件中 最后三个是磁力计的
...
void IMU_getValues(volatile float * values) {  
		...
		int i;
	//读取加速度和陀螺仪的当前ADC
    MPU6050_getMotion6(&accgyroval[0], &accgyroval[1], &accgyroval[2], &accgyroval[3], &accgyroval[4], &accgyroval[5]);//现有程序仅仅读取陀螺仪数据
	...
	for(i = 0; i<6; i++) {
		if(i < 3) {
			...
			values[i] = ((float) accgyroval[i]) *0.03048780f; //转成度每秒 0.03048780f = /32.8
